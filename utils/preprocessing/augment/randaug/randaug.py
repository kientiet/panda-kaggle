...
from utils.preprocessing.augment.adjust_transform import AdjustTransform
...
def random_flip(x):
  ...
  return transforms.RandomHorizontalFlip(p = 0.5)(x)


def zero_pad_and_crop(img, amount = 4):
  random_crop = transforms.RandomCrop(img, padding = amount)
...
def apply_policy(policy, pil_img, use_mean_std=True):
  ...
  pil_img = pil_wrap(np_img, use_mean_std = False)
  for xform in policy:
    assert len(xform) == 3
...
def _color_distortion_impl(img, s = 1.0):
    ...
    return color_distort(img)

color_distortion = TransformT("ColorDistortion", _color_distortion_impl)
