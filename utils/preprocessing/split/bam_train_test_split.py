...
import pandas as pd
import numpy as np
from utils.preprocessing.dataset.load_bam_dataset import BamPandaDataset
...
def split_dataframe(table, teacher = False):
  test = pd.DataFrame(columns = table.columns)
  for grade in table["isup_grade"].unique():
    if teacher:
      test = test.append(table[table["isup_grade"] == grade].sample(n = test_sample // 2, random_state = random_state))
    else:
...
def load_train_data(train_table, transform, data_dir, loss_type, label_type, teacher = False, **kwags):
  train_table = train_table.drop_duplicates(keep = "first")
  ...
  if teacher:
    kwags.pop("sample")
    return SupervisedDataset(new_train_table, transform = transform, data_dir = data_dir, loss_type = loss_type, **kwags)

  return BamPandaDataset(new_train_table, transform = transform, data_dir = data_dir, loss_type = loss_type, label_type = label_type, **kwags)


def load_test_data(test_table, transform, data_dir, loss_type, label_type, teacher = False):
  if teacher:
    return SupervisedDataset(test_table, transform = transform, data_dir = data_dir, loss_type = loss_type)

  return BamPandaDataset(test_table, transform = transform, data_dir = data_dir, loss_type = loss_type, label_type = label_type)
...
def load_for_teacher(transform, data_dir, loss_type, data_provider, **kwags):
  ...
  train, test = split_dataframe(table, teacher = True)
  trainset = load_train_data(train, transform[0], data_dir, loss_type, None, teacher = True, **kwags)
  testset = load_test_data(test, transform[1], data_dir, loss_type, None, teacher = True)
