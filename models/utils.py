...
def custom_load_state_dict(model, state_dict):
  ...
  for name in model_state_dict.keys():
    if "bn" in name:
      bn_name = name.split(".")
      if bn_name[0] == "_blocks":
        del bn_name[-2]
      else:
        del bn_name[1]
      bn_name = ".".join(bn_name)
      model_state_dict[name].copy_(state_dict[bn_name])
    elif name in state_dict:
      model_state_dict[name].copy_(state_dict[name])
...
def load_pretrained_weights(model, model_name, weights_path=None, load_fc=False, advprop=False):
    ...
    if load_fc:
        state_dict = custom_load_state_dict(model, state_dict)
