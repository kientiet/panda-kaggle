...
from utils.preprocessing.split.bam_train_test_split import load_for_bam_student
...
class StudentMultiTrainer(DistillTrainerSkeleton):
  ...
  def forward(self, images, hard_labels, soft_labels, running_mode):
    ...
    if running_mode == "training":
      loss, hard_loss, soft_loss, coeff = self.loss_func(logits, hard_labels, soft_labels, running_mode = running_mode)
  ...
  def training_step(self, train_batch, batch_idx):
    images, hard_labels, soft_labels = train_batch
    loss, hard_loss, soft_loss, coeff = self.forward(images, hard_labels, soft_labels, "training")
  ...
  def validation_step(self, val_batch, val_idx):
    images, hard_labels, soft_labels = val_batch
    val_loss, logits  = self.forward(images, hard_labels, soft_labels, "validation")
